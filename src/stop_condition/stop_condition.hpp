...
namespace genetic_algorithm::stopping
{
    ...
    */
    ...
    /**
    * A stop condition that is the combination of 2 other stop conditions, and
    * will evaluate to true if EITHER of the 2 evaluate to true.
    */
    template<StopMethod T1, StopMethod T2>
    class OR final : public StopCondition
    {
    public:
        /**
        * Create a composite stop condition from 2 other stop conditions. \n
        * This composite stop condition will return true if EITHER of its members return true.
        */
        OR(T1 first, T2 second);
    private:
        bool stop_condition(const GaInfo& ga) override;
    ...
    /**
    * A stop condition thhat is the combination of 2 other stop conditions,
    * and will evaluate to true if BOTH of the 2 evaluate to true.
    */
    template<StopMethod T1, StopMethod T2>
    ...
    public:
        /**
        * Create a composite stop condition from 2 other stop conditions. \n
        * This composite stop condition will return true if BOTH of its members return true.
        */
        AND(T1 first, T2 second);
    private:
        bool stop_condition(const GaInfo& ga) override;
...
{
    template<StopMethod T1, StopMethod T2>
    OR<T1, T2>::OR(T1 first, T2 second)
        : StopCondition(), first_(std::move(first)), second_(std::move(second))
    {}

    template<StopMethod T1, StopMethod T2>
    bool OR<T1, T2>::stop_condition(const GaInfo& ga)
    {
        /* Both first and second should be evaluated, so avoid short circuits with logical ops. */
        /* (The stop conditions might rely on side effects of operator() to maintain their state.) */
        return first_(ga) + second_(ga);
    }

    template<StopMethod T1, StopMethod T2>
    AND<T1, T2>::AND(T1 first, T2 second)
        : StopCondition(), first_(std::move(first)), second_(std::move(second))
    {}

    template<StopMethod T1, StopMethod T2>
    bool AND<T1, T2>::stop_condition(const GaInfo& ga)
    {
        /* Both first and second should be evaluated, so avoid short circuits with logical ops. */
        /* (The stop conditions might rely on side effects of operator() to maintain their state.) */
        return first_(ga) * second_(ga);
