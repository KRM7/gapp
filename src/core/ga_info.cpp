...
namespace gapp
{
    ...
    {
        if (!algorithm_) algorithm_ = std::make_unique<algorithm::AnyObjective<>>();
    ...
    {
        algorithm_ = f ? std::move(f) : std::make_unique<algorithm::AnyObjective<>>();
    }

    void GaInfo::algorithm(std::nullptr_t)
    {
        algorithm_ = std::make_unique<algorithm::AnyObjective<>>();
    }
