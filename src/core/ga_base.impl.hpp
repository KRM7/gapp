...
namespace genetic_algorithm
{
    ...
    void GA<T>::initializeAlgorithm(const Population<T>& initial_population)
    {
        ...
        population_ = generatePopulation(population_size_, initial_population);
    ...
    Population<T> GA<T>::generatePopulation(size_t pop_size, const Population<T>& initial_population) const
    {
        ...
        const size_t npreset = std::min(pop_size, initial_population.size());
        std::copy_n(initial_population.begin(), npreset, std::back_inserter(population));

        while (population.size() < pop_size)
        {
            population.push_back(generateCandidate());

            if (!hasValidChromosome(population.back()))
            {
                GA_THROW(std::logic_error, "An invalid solution was generated by generateCandidate().");
            }
        }
    ...
    void GA<T>::repair(Candidate<T>& sol) const
    {
        ...
        if (!hasValidChromosome(sol)) GA_THROW(std::logic_error, "Invalid chromosome returned by the repair function.");
    }
    ...
    template<Gene T>
    inline Candidates<T> GA<T>::run(const Population<T>& initial_population)
    {
        return run(initial_population, max_gen());
    }
    ...
    template<Gene T>
    Candidates<T> GA<T>::run(const Population<T>& initial_population, size_t num_generations)
    {
        max_gen(num_generations);

        initializeAlgorithm(initial_population);
        while (!stopCondition())
        {
            advance();
        }
